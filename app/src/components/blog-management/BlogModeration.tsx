    ...
    >
      ...
      <DialogContent className="min-w-[60vw] max-w-[90vw] max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button variant="default" className="bg-green-500" onClick={async () => {
            const isClose = await handleApproveBlog(blog._id);
            isClose && setIsOpen(false);
          }}>
            Chấp thuận
          </Button>
          <Button variant="destructive" onClick={async () => {
            const isClose = await handleRejectBlog(blog._id);
            isClose && setIsOpen(false);
          }}>Từ chối</Button>
        </DialogFooter>
      </DialogContent>
